...
class HttpRequest {
  ...
  updateOne = async (urlRoute, formData) => {
    const response = await Axios.put(this.constructUrl(urlRoute), formData, {
      headers: this._headers,
    });
    return response.data;
  };
  ...
  getBlob = async (urlRoute, id) => {
    return await Axios.get(`${HOST_URL}/api/v1/${urlRoute}`, {
      headers: { ...this._headers },
      responseType: "blob",
    });
  };
